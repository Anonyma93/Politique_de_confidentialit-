...
/**
 * Fonction déclenchée : Envoyer des notifications push quand un post reçoit une confirmation
 *
 * Cette fonction :
 * - Se déclenche automatiquement quand un post est mis à jour dans Firestore
 * - Vérifie si le champ confirmedBy a été modifié (nouvelle confirmation ajoutée)
 * - Envoie une notification push à l'auteur du post (s'il a un token)
 * - Ne notifie PAS l'auteur du post s'il se confirme lui-même
 */
exports.sendNotificationOnNewConfirmation = onDocumentUpdated(
  {
    document: 'posts/{postId}',
    region: 'europe-west1',
  },
  async (event) => {
    try {
      ...
      // Vérifier si confirmedBy a changé
      const beforeConfirmations = beforeData.confirmedBy || [];
      const afterConfirmations = afterData.confirmedBy || [];

      // Si aucune nouvelle confirmation, ne rien faire
      if (afterConfirmations.length <= beforeConfirmations.length) {
        return { success: true, notificationsSent: 0, skipped: 'no-new-confirmation' };
      }

      // Trouver la nouvelle confirmation
      const newConfirmations = afterConfirmations.filter(uid => !beforeConfirmations.includes(uid));
      if (newConfirmations.length === 0) {
        return { success: true, notificationsSent: 0, skipped: 'no-new-confirmation' };
      }

      // Prendre seulement la dernière confirmation
      const confirmerUserId = newConfirmations[newConfirmations.length - 1];

      console.log(`✅ Nouvelle confirmation sur post ${postId} par ${confirmerUserId}`);

      // Ne pas notifier si l'auteur confirme son propre post
      if (confirmerUserId === afterData.userId) {
        console.log('⏭️ L\'auteur a confirmé son propre post, pas de notification');
        return { success: true, notificationsSent: 0, skipped: 'self-confirmation' };
      }
      ...
      // Récupérer les infos du confirmeur
      const confirmerRef = db.collection('users').doc(confirmerUserId);
      const confirmerDoc = await confirmerRef.get();
      const confirmerName = confirmerDoc.exists ? (confirmerDoc.data().displayName || 'Un utilisateur') : 'Un utilisateur';

      // Créer le message de notification
      const confirmationsCount = afterData.confirmationsCount || afterConfirmations.length;
      const message = {
        to: postAuthorData.expoPushToken,
        sound: 'default',
        title: `${confirmerName} a confirmé votre post`,
        body: `${afterData.line} - ${afterData.station} (${confirmationsCount} confirmation${confirmationsCount > 1 ? 's' : ''})`,
        data: {
          type: 'confirmation',
          postId: postId,
          userId: confirmerUserId,
          userDisplayName: confirmerName,
        },
        badge: 1,
